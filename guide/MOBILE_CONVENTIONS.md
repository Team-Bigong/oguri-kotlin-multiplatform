...
- MVP 현재 구현에서는 메인 탭 이동 시 이전 백스택을 제거한다. (탭 전환 히스토리 미유지)
- 상세 화면은 탭 라우트 밖에서 `push`로 이동한다.
- 화면 전환 애니메이션은 과한 이동 애니메이션을 사용하지 않고, 공통 페이드 인/아웃만 적용한다. (MVP 기준)
- 메인 탭(`Home`, `Calendar`, `My`)에서 시스템 뒤로가기는 2회 입력 시 앱 종료로 처리한다.
  - 1회차: 상단 스낵바 안내
  - 2회차(약 2초 이내): 앱 종료
- iOS에서는 좌측 가장자리 스와이프(back gesture)로 백스택 이동을 지원한다.
  - 메인 탭 루트에서는 스와이프 백을 무시한다.
  - 비메인 화면(상세/문의 등)에서는 `popBackStack()`으로 처리한다.
