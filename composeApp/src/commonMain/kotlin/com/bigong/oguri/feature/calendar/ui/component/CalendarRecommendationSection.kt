...
private const val INITIAL_INLINE_BANNER_AFTER_CARD_INDEX: Int = 3
private const val INLINE_BANNER_INTERVAL: Int = 12
...
fun LazyListScope.calendarRecommendationSection(
    ...
    items(
        ...
        key = { index ->
            val periodCard: CalendarPeriodCardUiModel? = pagedPeriodCards.peek(index)
            periodCard?.id ?: "calendar_period_placeholder_$index"
        },
    ) { index ->
        val periodCard: CalendarPeriodCardUiModel? = pagedPeriodCards[index]
        if (periodCard == null) {
            if (index == pagedPeriodCards.itemCount - 1 && isLoadingNextPage) {
                Column(
                    modifier =
                        Modifier
                            .fillMaxWidth()
